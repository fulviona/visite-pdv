/* =========================================================
   VISITE PDV – v3.3
   Logica completa applicazione (BLOCCO 1/4)
   ========================================================= */
...
/* Applico suono + vibrazione a tutti i bottoni */
...
async function reverseGeocode(lat, lng) {
    ...
    const a = j?.address;
    return a
        ? `${a.road || ""}, ${a.city || a.town || a.village || ""}, ${a.postcode || ""}, Italia`
        : "";
}
...
/* =========================================================
   VISITE PDV – v3.3
   BLOCCO 2/4 — Lista PDV, Modifica, Import, Salvataggio
========================================================= */
...
/* =========================================================
   VISITE PDV – v3.3
   BLOCCO 3/4 — Mappa, Mini-Scheda, Navigazione, PDF
========================================================= */
...
function buildDeepLink(app, coord) {
    ...
    switch (app) {
        case 'google':
            return `comgooglemaps://?daddr=${latlng}&directionsmode=driving`;
        case 'waze':
            return `waze://?ll=${latlng}&navigate=yes`;
        case 'apple':
            return `maps://?daddr=${latlng}&dirflg=d`;
        case 'osm':
            return `https://www.openstreetmap.org/directions?engine=fossgis_osrm_car&route=${encodeURIComponent(latlng)}`;
        default:
            return `comgooglemaps://?daddr=${latlng}&directionsmode=driving`;
    }
}
...
function askNavigationApp(orderedPts) {
    if (!navigationFlowActive) return; // Sicurezza iPhone
...
async function exportPDF() {
    ...
    // Rimuove polyline per evitare distorsioni
    if (layerRoute) {
    ...
    for (const v of visite) {
        ...
        pdf.text(`PDV: ${v.nome || ''}`, 10, y);
        y += 6;

        if (v.address) {
            pdf.text(`Indirizzo: ${v.address}`, 10, y);
            y += 6;
        }
        ...
        // Recupero coordinate se mancanti
        ...
        if (isFinite(lat) && isFinite(lng)) {
            pdf.text(
                `Lat: ${Number(lat).toFixed(6)}  Lng: ${Number(lng).toFixed(6)}  (${v.src === 'gps' ? 'GPS' : 'Auto'})`,
                10, y
            );
        } else {
    ...
    // Ripristina percorso sulla mappa
    if (restoreRoute) {
        const src = visite.filter(v => v.lat && v.lng)
...
/* =========================================================
   VISITE PDV – v3.3
   BLOCCO 4/4 — Init, Eventi, Loading Screen
========================================================= */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    el = {
        ...
        istr: document.getElementById('istruzioni'),   // istruzioni rimosse da CSS
    ...
    el.btnNav.onclick = () => {
        ...
        if (!src.length)
            return alert("Nessun punto con coordinate valide.");
